...
- `series-daily/station-<id>.json` — per-day `[idx, mean, min, max, n]` per substance
  spanning all years; drives the map's date-range averaging and the Day/Week/Month/Year
  chart intervals. **Committed.**
- `series-profiles/station-<id>.json` — per **local calendar year**, weekday/weekend ×
  hour-of-day value sums and sample counts; drives the Weekday/Weekend diurnal
  profiles over ranges longer than the hourly bound. **Committed.**
...
static/
  ...
  data/               Compact JSON served to the app (preprocess.py output)
                      series-daily/ + series-profiles/ committed; series/ (hourly) git-ignored
```
...
The committed `static/data/` (daily series + diurnal profiles + station/meta files)
is enough to run the app without the archive; `fetch-archive.sh` + `preprocess.py`
are only needed to regenerate data or to produce the hourly tier that the Hour
interval and short-range diurnal profiles load on demand.
